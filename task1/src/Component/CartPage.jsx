  ...
  const updateQty = async (id, action) => {
    try {
      if (user_id) {
        // backend update
        await axios.put(
          `http://localhost:5000/api/cart/${action}/${id}`,
          { user_id }
        );
      } else {
        // guest update
        ...
        if (action === "increase") {
          guestCart[id] = (guestCart[id] || 0) + 1;
        } else {
          guestCart[id] = (guestCart[id] || 0) - 1;
          if (guestCart[id] <= 0) delete guestCart[id];
        }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-3 gap-10">
                ...
                {cartItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[#008744] font-bold">
                        ₹{item.price}
                      </p>
            ...
            {/* RIGHT - SUMMARY */}
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 h-fit sticky top-32">
              ...
              <button
                onClick={() => navigate("/checkout")}
                className="w-full mt-6 bg-[#008744] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2"
              >
                <CreditCard size={18} />
                Proceed to Checkout
              </button>
